...
<table id="example1" class="table table-bordered table-striped">
    ...
    <tr>
      ...
      <?php if($dosen->id_dosen == $i->ketua_kompre):?>
        <td>Ketua Penguji</td>
      <?php elseif($dosen->id_dosen == $i->sekre_kompre): ?>
        <td>Sekretaris Penguji</td>
      <?php elseif($dosen->id_dosen == $i->anggota_kompre): ?>
      ...
      <?php 
        ...
        $str_jadwal = strtotime($i->jadwal_sidang_kompre);
      ?>

      <?php if($str > $str_jadwal): ?>
      <?php elseif($str < $str_jadwal ): ?>
        <td><button class="btn btn-danger btn-sm">Persidangnan Belum Mulai</button></td>
      <?php else:?>
        <td><a href="<?php echo base_url('dosen/sidang/ruang_sidang_kompre/'.$i->id_mahasiswa) ?>" class="btn btn-info btn-sm"><i class="fa fa-gavel"></i> Sidang</a></td>
      <?php endif; ?>
    </tr>
    <!-- <h1>Tanggal sekarang = <?=date('Y-m-d H:i:s')?> </h1>
    <h1>Tanggal database = <?=$i->jadwal_sidang_kompre?></h1> -->
<?php } ?>
  </tbody>
</table>
</section>
